...
class NodeGameState(GameState):
    ...
    def other_player_squares(self):
        """
        Returns the squares where the not current player can play, or None if all squares are allowed.
        """
...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.transposition_table = {} # dictionary for the transposition table
    ...
    def apply_move(self, node, move):
        ...
        if type(move) != str:
            ...
            if new_node.current_player == 1:
                ...
                return new_node
        # if node is blocked only switch current player
        else:
    ...
    def calculate_move_score(self, node, move):
        ...
        """
        N = node.board.N
        board = [node.board.squares[i * N:(i + 1) * N] for i in range(N)] # construct the board matrix

        n, m = node.board.n, node.board.m  # Block dimensions
        x, y = move.square  # Get the row and column of the move

        row_complete = all(cell != 0 for cell in board[x]) #check if row completed
        col_complete = all(row[y] != 0 for row in board) # check if column completed
        ...
        rows = board[block_row:block_row + m]
        block = [row[block_col:block_col + n] for row in rows]
        block_complete = all(cell != 0 for row in block for cell in row)

        # assign score
        regions_completed = int(row_complete) + int(col_complete) + int(block_complete)
    ...
    def minimax(self, node, depth, is_maximizing_player, alpha, beta):
        ...
        if depth == 0 or self.is_terminal(node):
            # get score from the transposition table if already calculated
            if node_key in self.transposition_table:
                score = self.transposition_table[node_key]
            else: # calculate the score and store the result
        ...
        # if player can not perform a move and game state is not terminal then player is blocked
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        N = root_node.board.N
        # change game phase to late if N or less empty squares left
