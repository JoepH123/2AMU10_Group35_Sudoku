...
def evaluate_node(node):
    ...
    if len(current_player_occupied) <= node.board.m:
        p=0
    else:
        p=1
    eval_func = score_diff_game + 2 * score_diff_center + score_one_empty + p*score_mobility + score_block + score_line
    #print('score_diff: ', score_diff_game, 'score_center: ', score_diff_center,  'score_one_empty: ', score_one_empty , 'mobility: ',p*score_mobility, 'score block occupation: ',score_block, 'score line: ',score_line,'eval: ' ,eval_func)
    return eval_func