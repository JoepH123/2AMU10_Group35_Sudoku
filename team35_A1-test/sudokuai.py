...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state):
        ...
        if num_occupied == 0 and node.my_player==1:
            self.propose_move(Move((0, node.board.N//2), node.board.N))
        elif num_occupied == 1 and node.my_player==1:
            self.propose_move(Move((1, node.board.N//2), node.board.N-1))
        elif num_occupied == 0 and node.my_player==2:
            self.propose_move(Move((node.board.N-1, node.board.N//2), node.board.N))
        elif num_occupied == 1 and node.my_player==2:
            self.propose_move(Move((node.board.N-2, node.board.N//2), node.board.N-1))
        else:
