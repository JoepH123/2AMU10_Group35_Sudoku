...
class Benchmark():
    ...
    def __init__(self, model, time=1, count=20, comparing_models = ["greedy_player", "random_player", "team35_with_randomness"], board_sizes=["empty-2x2.txt", "empty-2x3.txt", "empty-3x3.txt", "empty-3x4.txt", "empty-4x4.txt"]):
        self.model = model
        self.time = time
    ...
    def compute_benchmark_results(self):
        ...
        file_path = f"benchmark_results/benchmark_{self.model}_{time_str}s_{self.count}count_{len(self.board_sizes)}boards_{len(self.comparing_models)}opps.csv"
    ...
    def play_match(self, player, opponent, board_sizes, verbose=False, warmup=False) -> None:
        ...
        for i in range(1, self.count+1):
            ...
            result = play_game(board_sizes, first, second, self.time, verbose, warmup and i == 1)

            if result[0] == 0.5:  # draw
                draw_score += 1
            else:  # non-draw score
                if player_starts:
        ...
        result_of_the_match = f'Match result: {player} - draw - {opponent} {self.print_score(player_score)}-{self.print_score(draw_score)}-{self.print_score(opponent_score)}'
        print(result_of_the_match)

        return f"{player_score} - {draw_score} - {opponent_score}"
...
if __name__ == '__main__':
    ...
    # benchmark = Benchmark('team35_A1', count=1, comparing_models = ["greedy_player"], board_sizes=["empty-2x2.txt"])  #, "empty-2x3.txt", "empty-3x3.txt"])
    
    # benchmark_01s = Benchmark('team35_A2', count=10, time=0.1, comparing_models = ["greedy_player", "team35_A1"], board_sizes=["empty-3x3.txt"])  #, "empty-2x3.txt", "empty-3x3.txt"])
    # benchmark_05s = Benchmark('team35_A2', count=10, time=0.5, comparing_models = ["greedy_player", "team35_A1"], board_sizes=["empty-3x3.txt"])  #, "empty-2x3.txt", "empty-3x3.txt"])
    # benchmark_1s = Benchmark('team35_A2', count=10, time=1, comparing_models = ["greedy_player", "team35_A1"], board_sizes=["empty-3x3.txt"])  #, "empty-2x3.txt", "empty-3x3.txt"])
    # benchmark_5s = Benchmark('team35_A2', count=10, time=5, comparing_models = ["greedy_player", "team35_A1"], board_sizes=["empty-3x3.txt"])  #, "empty-2x3.txt", "empty-3x3.txt"])

    # All results are then computed and timed
    start = time.perf_counter()
    # results = benchmark_1s.compute_benchmark_results()
