...
def simulate_game(game_state: GameState,
    ...
    with multiprocessing.Manager() as manager:
        ...
        scores_obtained_per_player = {1: [], 2: []}
        ...
        while move_number < number_of_moves and len(finished_players) < 2:
            ...
            if player_squares == []:
                ...
                if (i, j, value) != (0, 0, 0):
                    ...
                    return (0, 1) if player_number == 1 else (1, 0)
            scores_obtained_per_player[player_number].append(player_score)
            ...
            if SudokuSettings.print_ascii_states:
                ...
                log(pretty_print_game_state(game_state))
        print("######## Point Frequencies ########")
        frequencies_points_scored = {key: dict(Counter(value for value in values if value != 0)) for key, values in scores_obtained_per_player.items()}
        print(frequencies_points_scored)
        if not verbose:
